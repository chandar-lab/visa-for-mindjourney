...
class PipelineBase:
    ...
    def __init__(
        ...
        num_questions = self.model_args.num_questions # 150 by default
        
        if self.model_args.question_type == "None":
            # this is the default case, load test.json for evaluation
        ...
        self.questions = self.select_questions(input_file, seed=10, num_questions=num_questions) # select at most 150 questions from test.json
        if self.model_args.num_question_chunks > 1:
    ...
    def run(self):
        ...
        for question in tqdm(self.questions):
            ...
            if len(question["img_paths"]) > self.model_args.max_images: # > 2 by default
            ...
            # by default, max_steps_per_question is 3
            for step in range(self.model_args.max_steps_per_question):
                print(f"\n[SpatialVQA] ===== Step {step} for QID {qid} =====")

                for _ in range(self.model_args.max_tries_gpt): # by default, max_tries_gpt is 5
                    ...
                    if self.model_args.compute_uncertainty and "InternVL" in self.model_args.vlm_model_name:
                        uncertainty_metrics = self.vlm.run_prompt("answer_baseline", sys_prompt, content, return_log_probs=True, answer_choices=question["answer_choices"])
                    print("[LLM]", response)
                    result = self._process_answer(response, question)
                    if result != "out of control":
                        break

                self._dump_llm_interaction(save_dir, step, question, response, result, None, None,  uncertainty_metrics=uncertainty_metrics)
                if result == "out of control":
                    result = "wrong"
                # {progress: {question_type: {correct: [qid1, qid2], wrong: [qid3, qid4]}}}
    ...
    def init_prompt(self, question):
        ...
        return new_prompt
    def _dump_llm_interaction(self, save_dir, step, question, response, result, actions, magnitude, uncertainty_metrics=None):
