...
def format_verification_prompt_claim_generation(
    ...
    content = [
        ...
        (f"BEFORE (previous view):", frames[0]),
        (f"AFTER (current view):", frames[1]),
...
def format_image_comparison_prompt_v2(
    action_description: str,
    images: List[str],
    question: str,
    n_claims: int = 3,
    ...
    """
    Format prompt for generating claims from image comparison.
    Optimized for MMSI-bench with question-type specific guidance.
    """
    sys_prompt = (
        "You are an AI assistant that analyzes spatial changes between two images. "
        "Generate 2-4 specific claims that help answer the spatial reasoning question. "
        "Focus on changes that discriminate between the answer choices. "
        "Only describe what you can clearly observe - avoid claims about objects not visible in the images."
    )
    ...
    # Add answer choices
    if answer_choices:
        ac_text = "Answer choices: " + ", ".join(answer_choices)
        content.append((ac_text,))
    
    # Question-type specific guidance (CONCISE)
    if question_type:
        guidance = _get_mmsi_claim_guidance(question_type, action_description, answer_choices, question)
        content.extend(guidance)
    else:
        # Fallback to general guidance
        content.extend([
            ("\nFocus on spatial changes that help distinguish between answer choices:",),
            ("- Camera/object motion direction (left, right, forward, backward)",),
            ("- Spatial relationships between objects",),
            ("- Perspective changes that indicate movement",),
            ("\nGenerate 2-3 claims (one per line, start with '- '):",),
        ])
    
    # Add critical evidence-based instruction
    content.extend([
        ("\nIMPORTANT: Only make claims about objects/features you can clearly see in the images.",),
        ("If an object mentioned in the question is not visible, do not make claims about it.",),
    ])
    
    return sys_prompt, content

def _get_mmsi_claim_guidance(question_type: str, action_description: str, answer_choices: Optional[List[str]], question: str) -> List[Tuple[str]]:
    """Return concise, question-type specific guidance for MMSI-bench claim generation."""
    
    if question_type == "motion_(cam.)":
        return [
            ("\nFor camera motion questions, focus on:",),
            ("- Camera rotation direction (clockwise/counterclockwise, left/right)",),
            ("- How scene perspective shifted (what became more/less visible)",),
            ("- Reference objects that indicate camera movement direction",),
            ("\nGenerate 2-3 claims (one per line, start with '- '):",),
        ]
    
    elif question_type == "positional_relationship_(cam.-obj.)":
        return [
            ("\nFor camera-object position questions, focus on:",),
            ("- Object visibility changes from camera movement",),
            ("- How camera rotation reveals object locations",),
            ("- Spatial relationships that help determine object positions",),
            ("\nGenerate 2-3 claims (one per line, start with '- '):",),
        ]
    
    elif question_type == "positional_relationship_(obj.-obj.)":
        return [
            ("\nFor object-object position questions, focus on:",),
            ("- Relative positions between visible objects",),
            ("- How objects are arranged spatially (left/right, front/back)",),
            ("- Clear spatial relationships that distinguish answer choices",),
            ("\nGenerate 2-3 claims (one per line, start with '- '):",),
        ]
    
    elif question_type == "msr":
        return [
            ("\nFor mirror/spatial reasoning questions, focus on:",),
            ("- What appears in mirrors vs real space",),
            ("- How reflections change object positions",),
            ("- Spatial transformations from mirror effects",),
            ("\nGenerate 2-3 claims (one per line, start with '- '):",),
        ]
    
    elif question_type == "attribute_(appr.)":
        return [
            ("\nFor appearance/attribute questions, focus on:",),
            ("- How object appearance changes with perspective",),
            ("- Which sides/angles of objects become visible",),
            ("- Spatial orientation changes that reveal attributes",),
            ("\nGenerate 2-3 claims (one per line, start with '- '):",),
        ]
    
    elif question_type == "motion_(obj.)":
        return [
            ("\nFor object motion questions, focus on:",),
            ("- Object rotation direction (clockwise/counterclockwise)",),
            ("- How object orientation changed (handle position, visible sides)",),
            ("- Clear rotation indicators that distinguish answer choices",),
            ("\nGenerate 2-3 claims (one per line, start with '- '):",),
        ]
    
    # Check for cardinal direction questions
    elif answer_choices and any(choice in ['northeast', 'northwest', 'southeast', 'southwest', 'north', 'south', 'east', 'west'] for choice in answer_choices):
        return [
            ("\nFor directional questions, focus on:",),
            ("- Object orientation changes (which side is now visible)",),
            ("- Rotation or perspective shifts",),
            ("- Landmarks that indicate cardinal directions",),
            ("\nGenerate 2-3 claims (one per line, start with '- '):",),
        ]
    
    # Check for goal-oriented questions
    elif "how should" in question.lower() or "how to" in question.lower():
        return [
            ("\nFor goal-oriented questions, focus on:",),
            ("- Where target objects are relative to current viewpoint",),
            ("- What camera movement would reach the target",),
            ("- Spatial relationships that guide the required action",),
            ("\nGenerate 2-3 claims (one per line, start with '- '):",),
        ]
    
    else:  # general_spatial
        return [
            ("\nFor spatial reasoning questions, focus on:",),
            ("- Camera/object motion direction (left, right, forward, backward)",),
            ("- Spatial relationships between objects",),
            ("- Perspective changes that indicate movement",),
            ("\nGenerate 2-3 claims (one per line, start with '- '):",),
