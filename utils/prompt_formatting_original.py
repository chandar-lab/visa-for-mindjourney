...
def format_image_comparison_prompt(
    ...
    content.extend([
        ...
        ("- Generate 2-4 claims based on the complexity and clarity of changes observed",),
        ("- If changes are minimal or unclear, generate fewer claims",),
        ("- If multiple clear changes are visible, you may generate up to 4 claims",),
        ("- Each claim should be independently verifiable and distinct",),
        ("- Prioritize the most important changes for answering the spatial reasoning question",),
        ("\nExamples of preferred patterns:",),
        ("- [Object X] [appears/disappears/moves into view/moves out of view] [specific location]",),
        ("- [Object X] [moves from position A to position B] relative to [reference point]",),
        ("- [Object X] [becomes more/less visible/occluded] at [specific edge or area] of the frame",),
        ("\nAVOID these unreliable claim types:",),
        ...
        ("- Multi-predicate sentences (split them)",),
        ...
        ("- Make claims that would help answer the spatial reasoning question",),
        ("- Generate only as many claims as you can confidently verify",),
        (f"Output the micro-claims, one per line, starting with '- '",)
    ])
